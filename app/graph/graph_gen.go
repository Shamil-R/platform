...
type ComplexityRoot struct {
	Material struct {
		...
		Author func(childComplexity int, where *model.UserWhereInput) int
	}
...
func field_Material_author_args(rawArgs map[string]interface{}) (map[string]interface{}, error) {
...
func (e *executableSchema) Complexity(typeName, field string, childComplexity int, rawArgs map[string]interface{}) (int, bool) {
	...
	case "Material.author":
		if e.complexity.Material.Author == nil {
			break
		}

		args, err := field_Material_author_args(rawArgs)
		if err != nil {
			return 0, false
		}

		return e.complexity.Material.Author(childComplexity, args["where"].(*model.UserWhereInput)), true
	...
	for i, field := range fields {
		...
		case "author":
			...
			go func(i int, field graphql.CollectedField) {
				out.Values[i] = ec._Material_author(ctx, field, obj)
				if out.Values[i] == graphql.Null {
					invalid = true
				}
				wg.Done()
...
// nolint: vetshadow
func (ec *executionContext) _Material_author(ctx context.Context, field graphql.CollectedField, obj *model.Material) graphql.Marshaler {
	...
	resTmp := ec.FieldMiddleware(ctx, obj, func(rctx context.Context) (interface{}, error) {
		ctx = rctx // use context from middleware stack in children
		return ec.resolvers.Material().Author(rctx, obj, args["where"].(*model.UserWhereInput))
	})
	if resTmp == nil {
		if !ec.HasError(rctx) {
			ec.Errorf(ctx, "must not be null")
...
func UnmarshalMaterialCreateInput(v interface{}) (model.MaterialCreateInput, error) {
	...
	for k, v := range asMap {
		...
		case "author":
			var err error
			it.Author, err = UnmarshalUserCreateOneWithoutMaterialsInput(v)
			if err != nil {
				return it, err
			}
		}
...
func UnmarshalUserCreateOneWithoutMaterialsInput(v interface{}) (model.UserCreateOneWithoutMaterialsInput, error) {
	var it model.UserCreateOneWithoutMaterialsInput
	var asMap = v.(map[string]interface{})
	...
	for k, v := range asMap {
		...
		case "connect":
			var err error
			var ptr1 model.UserWhereUniqueInput
			if v != nil {
				ptr1, err = UnmarshalUserWhereUniqueInput(v)
				it.Connect = &ptr1
			}
...
type Material {
	...
	author(where: UserWhereInput): User! @field(name: "user_id")
}
...
input MaterialCreateInput {
	name: String! @validate(min: 10 max: 20)
	author: UserCreateOneWithoutMaterialsInput! @field(name: "user_id")
}
